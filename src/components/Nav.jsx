    ...
    <>
      <nav className={`px-6 ${showSearchBar? 'pt-2 pb-2' : 'pt-6 pb-5'}  flex items-center justify-between text-lg fixed top-0 w-full bg-white z-50`}>
        ...
        <div className={`flex flex-row items-center ${showSearchBar ? 'hidden md:flex' : 'flex'}`}>
          <Link to="/pixterest/" className="flex items-center cursor-pointer">
          ...
          <ul className="hidden md:flex flex-row gap-8 mx-4 items-center">
            <li className="hover:underline"><Link to="/pixterest/today">Today</Link></li>
            <li className="hover:underline"><Link to="/pixterest/explore">Explore</Link></li>
          </ul>
        </div>
        {showSearchBar && <SearchBar />}
        <div className="hidden md:flex flex-row gap-4 items-center">
          {!showSearchBar && <button className="hover:underline"><Link to="/pixterest/about">About</Link></button>}
          <Link to="/pixterest/myideas">
      ...
      <div className={`fixed flex flex-col top-0 left-0 h-full w-64 bg-white shadow-lg transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out z-40`}>
        <ul className="flex flex-col gap-8 mx-4 mt-20 items-start">
          <li className="hover:underline"><Link to="/pixterest/today" onClick={toggleMenu}>Today</Link></li>
          <li className="hover:underline"><Link to="/pixterest/explore" onClick={toggleMenu}>Explore</Link></li>
          <li className="hover:underline"><Link to="/pixterest/about" onClick={toggleMenu}>About</Link></li>
          <li className="hover:underline"><Link to="/pixterest/myideas" onClick={toggleMenu}>My Ideas</Link></li>
          <li className="hover:underline" onClick={openSettings}><button>Settings</button></li>
        </ul>
        {showSearchBar &&
        <Link to="/pixterest/" className="flex items-center cursor-pointer mt-auto">
